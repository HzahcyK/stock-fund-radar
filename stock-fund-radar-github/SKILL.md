...
description: Daily stock and fund market research skill for A-share investors and fund allocators. Use when the user asks for today's A-share market, overseas/peripheral markets, global politics, China policy support, main capital flows, market direction, sector rotation, mainline/theme prediction, quietly-starting sectors, ETF/index/active/QDII fund screening, fund recommendations, portfolio watchlists,财报/业绩/估值分析, or iterative investing workflows in Chinese or English such as 今日A股、外围怎么看、主线方向、资金流、板块挖掘、基金筛选、炒股、养基金、财报、业绩预期、估值、深挖机会.
---
...
2. Build a current data pack. Read `references/data-checklist.md` when the task involves today's market, capital flows, policies, or overseas markets.
3. If the user asks for主线、埋伏、重仓/加仓、深挖, or criticizes depth, read `references/research-depth.md` and run the deeper evidence-chain workflow.
4. Score market regime and themes. Read `references/scoring-rubric.md` when ranking sectors, timing themes, or selecting funds.
5. For重点板块, confirm technical structure with K-line, moving averages, volume/price behavior, support/resistance, and momentum indicators before ranking.
6. Produce a concise decision-grade brief. Read `references/output-template.md` when the user asks for a full daily report or fund shortlist.
7. Incorporate personal constraints from `references/user-preferences.md` before final recommendations.
...
- Technical picture for重点板块: daily and weekly K-line trend, MA5/10/20/60 position, volume expansion/shrinkage, MACD/KDJ/RSI if available, gap/support/resistance, relative strength versus major indices, and whether ETF price action confirms the underlying sector.
